...
int sno=0;
	...
	%>
<h3>Online shopping (RHMATE STORE)</h3>
<hr>
<div class="left-div"><h3>Name:<%=rs2.getString(1) %>  </h3></div>
<div class="right-div-right"><h3>Email: <%out.print(email); %> </h3></div>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs=st.executeQuery("select *from cart inner join product on cart.product_id=product.id where cart.email='"+email+"' and cart.status='bill' ");
	while(rs.next())
	{
		sno=sno+1;
		%>
